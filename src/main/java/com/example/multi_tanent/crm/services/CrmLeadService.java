...
import com.example.multi_tanent.crm.repository.LeadSourceRepository;
    ...
    public class CrmLeadService {
        ...
        public CrmLeadResponse updateLead(Long id, CrmLeadRequest request) {
            ...
            if (!lead.getLeadNo().equalsIgnoreCase(request.getLeadNo()) && leadRepository.existsByTenantIdAndLeadNo(currentTenant().getId(), request.getLeadNo())) {
                throw new IllegalArgumentException("Lead with number '" + request.getLeadNo() + "' already exists for this tenant.");
            }
        ...
        private void mapReqToEntity(CrmLeadRequest r, CrmLead e) {
            ...
            if (r.getIndustryId() != null) e.setIndustry(industryRepository.findById(r.getIndustryId()).orElse(null));
            if (r.getOwnerId() != null) e.setOwner(employeeRepository.findById(r.getOwnerId()).orElse(null));
            if (r.getCurrentStageId() != null) e.setCurrentStage(leadStageRepository.findById(r.getCurrentStageId()).orElse(null));
            if (r.getLeadSourceId() != null) {
                LeadSource leadSource = leadSourceRepository.findById(r.getLeadSourceId())
                        .orElseThrow(() -> new EntityNotFoundException("LeadSource not found with id: " + r.getLeadSourceId()));
                e.setLeadSource(leadSource);
            }
            if (r.getLocationId() != null) e.setLocation(locationRepository.findById(r.getLocationId()).orElse(null));
            if (r.getProductIds() != null) e.setProducts(new HashSet<>(productRepository.findAllById(r.getProductIds())));
        }
        ...
        private CrmLeadResponse toResponse(CrmLead e) {
            return CrmLeadResponse.builder()
                    ...
                    .leadNo(e.getLeadNo())
                    .companyId(e.getCompany() != null ? e.getCompany().getId() : null).companyName(e.getCompany() != null ? e.getCompany().getName() : null)
                    .industryId(e.getIndustry() != null ? e.getIndustry().getId() : null).industryName(e.getIndustry() != null ? e.getIndustry().getName() : null)
                    .designation(e.getDesignation()).phone(e.getPhone()).email(e.getEmail()).website(e.getWebsite())
                    .products(e.getProducts().stream().map(p -> new CrmLeadResponse.ProductMini(p.getId(), p.getName())).collect(Collectors.toSet()))
                    ...
                    .ownerId(e.getOwner() != null ? e.getOwner().getId() : null).ownerName(e.getOwner() != null ? e.getOwner().getFirstName() + " " + e.getOwner().getLastName() : null)
                    .currentStageId(e.getCurrentStage() != null ? e.getCurrentStage().getId() : null).currentStageName(e.getCurrentStage() != null ? e.getCurrentStage().getName() : null)
                    .locationId(e.getLocation() != null ? e.getLocation().getId() : null).locationName(e.getLocation() != null ? e.getLocation().getName() : null)
                    // 
