...
public class PdfBenefitVoucherService {
    ...
    private void addHeader(Document document, CompanyInfo companyInfo) {
        ...
        if (companyInfo != null) {
            headerTable.addCell(new Cell().add(new Paragraph(companyInfo.getCompanyName()).setFont(boldFont).setFontSize(18)).setBorder(Border.NO_BORDER));
        }
        headerTable.addCell(new Cell().add(new Paragraph("Benefit Payout Voucher").setFont(boldFont).setFontSize(14)).setBorder(Border.NO_BORDER));
    ...
    private void addDetailsTable(Document document, EmployeeBenefitProvision provision, Employee employee) {
        ...
        detailsTable.addCell(createDetailCell("Date:", provision.getPaidOutDate().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))));
        ...
        detailsTable.addCell(createDetailCell("Benefit Cycle:", provision.getCycleStartDate() + " to " + provision.getCycleEndDate()));
    ...
    private void addSignatureSection(Document document) {
        ...
        signatureTable.addCell(new Cell().add(new Paragraph("\n\n____________________\nReceived By (Employee)").setFont(regularFont)).setBorder(Border.NO_BORDER));
        signatureTable.addCell(new Cell().add(new Paragraph("\n\n____________________\nAuthorized Signatory").setTextAlignment(TextAlignment.RIGHT).setFont(regularFont)).setBorder(Border.NO_BORDER));
