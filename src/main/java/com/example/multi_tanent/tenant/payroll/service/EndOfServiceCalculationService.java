...
public class EndOfServiceCalculationService {
    ...
    public EndOfService calculateAndSaveGratuity(String employeeCode, LocalDate lastWorkingDay, TerminationReason reason) {
        ...
        JobDetails jobDetails = jobDetailsRepository.findByEmployeeId(employee.getId())
                .orElseThrow(() -> new IllegalStateException("JobDetails not found for employee: " + employeeCode));
        ...
        BigDecimal lastBasicSalary = structure.getComponents().stream()
                ...
                .findFirst()
                .orElseThrow(() -> new IllegalStateException("Basic salary component not found in structure."))
                .getValue();
