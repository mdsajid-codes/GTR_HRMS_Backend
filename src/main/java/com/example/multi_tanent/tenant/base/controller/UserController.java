...
import com.example.multi_tanent.tenant.base.dto.UserRegisterRequest;
...
public class UserController {
  ...
  public ResponseEntity<?> create(@RequestBody UserRegisterRequest uRequest) {
    if (repo.findByEmail(uRequest.getEmail()).isPresent()) {
      return ResponseEntity.status(HttpStatus.CONFLICT)
              .body("User with email '" + uRequest.getEmail() + "' already exists.");
    }

    String currentUsername = SecurityContextHolder.getContext().getAuthentication().getName();
    User currentUser = repo.findByEmail(currentUsername)
            .orElseThrow(() -> new IllegalStateException("Could not find currently authenticated user to determine tenant plan."));
    TenantPlan plan = currentUser.getPlan();
  ...
  @PutMapping("/{id}")
  @PreAuthorize("hasAnyRole('TENANT_ADMIN','HR')")
  public ResponseEntity<?> update(@PathVariable Long id, @RequestBody UserRegisterRequest uRequest) {
    Optional<User> existingByEmail = repo.findByEmail(uRequest.getEmail());
    if (existingByEmail.isPresent() && !existingByEmail.get().getId().equals(id)) {
        return ResponseEntity.status(HttpStatus.CONFLICT)
                .body("Another user with the email '" + uRequest.getEmail() + "' already exists.");
    }
